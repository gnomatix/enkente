...
*   **Structured Methodology Support (Auto-Recognition):** The semantic engine must be capable of *automatically recognizing* when established methodologies or conversational patterns are being used (e.g., *Socratic Method*, *Six Thinking Hats*, *SCAMPER*, *Five Whys*, *Brainwriting*, narrative storytelling, or analogies). Identifying these patterns must happen in real-time. Furthermore, the engine must be reflexive and self-optimizing, using these recognized structures to continuously improve its own internal semantic processing performance and route ideas appropriately based on the recognized phase or pattern.
